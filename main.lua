...
-- СИСТЕМА ДАННЫХ И КОНФИГ
local CONFIG_NAME = "LiteVisuals_Settings.json"
local settingsData = {
    walkSpeed = 16, flySpeed = 50, spinSpeed = 40, magnetSize = 10,
    noclip = false, autoSave = true, antiFling = false, skinHack = false,
    speedHackActive = false, flyActive = false, spinActive = false,
    magnetEnabled = false, espEnabled = false,
    espNames = false, espMM2 = false, rgb = false, graphics = false, fov = 70,
    infJump = false, bHop = false, fullbright = false, spammer = false
}

local function saveConfig()
    if not settingsData.autoSave then return end
    pcall(function() writefile(CONFIG_NAME, HttpService:JSONEncode(settingsData)) end)
end

local function loadConfig()
    if isfile and isfile(CONFIG_NAME) then
        local success, decoded = pcall(function() return HttpService:JSONDecode(readfile(CONFIG_NAME)) end)
        if success then 
            for k, v in pairs(decoded) do settingsData[k] = v end 
        end
    end
end
loadConfig()

-- ТВИНИНГ И ОБНОВЛЕННЫЙ ИСПРАВЛЕННЫЙ ДРАГГИНГ
...
local Localization = {
    RU = {
        MainTab = "Читы", VisualsTab = "Визуалы", UniversalTab = "Универсал", SettingsTab = "Настройки", ActiveModsTitle = "АКТИВНО:",
        ...
        Noclip = "Noclip", Esp = "Neon ESP (Wallhack)", EspNames = "ESP Никнеймы", 
        EspMM2 = "ESP Роли (MM2 Авто)", Fov = "FOV (Растяг)", Widget = "Мини-виджет (Stats)", 
        ActiveList = "Список включенного", Rainbow = "Rainbow Glow", LangBtn = "Язык: RU", 
        Graphics = "Улучшенная графика (RTX)", Magnet = "Магнит пуль (Hitbox)",
        AntiFling = "Anti-Fling (Улучшенный)", SkinHack = "Скин Хак (Для себя)", AutoSave = "Автосохранение",
        InfJump = "Бесконечный прыжок", BHop = "Auto-Bhop (Распрыжка)", 
        Fullbright = "Fullbright (Ночное зрение)", Spammer = "Авто-спамер чата", ClickTP = "Ctrl + Клик = Телепорт"
    },
    EN = {
        MainTab = "Main Hacks", VisualsTab = "Visuals", UniversalTab = "Universal", SettingsTab = "Settings", ActiveModsTitle = "ACTIVE MODS:",
        ...
        Noclip = "Noclip", Esp = "Neon ESP (Wallhack)", EspNames = "ESP Names", 
        EspMM2 = "ESP Roles (MM2 Auto)", Fov = "FOV Tracker", Widget = "Stats Widget (Drag)", 
        ActiveList = "Active Mods Widget", Rainbow = "Rainbow Glow", LangBtn = "Lang: EN", 
        Graphics = "Enhance Graphics (RTX)", Magnet = "Bullet Magnet (Hitbox)",
        AntiFling = "Anti-Fling (Advanced)", SkinHack = "Skin Changer", AutoSave = "Auto-Save Config",
        InfJump = "Infinite Jump", BHop = "Auto-Bhop (Velocity)", 
        Fullbright = "Fullbright (No Dark)", Spammer = "Chat Auto-Spammer", ClickTP = "Ctrl + Click = Teleport"
    }
}
...
-- Привязываем драггинг напрямую к фрейму Main, управляя положением CanvasGroup
...
local VisualsTab = createTab("VisualsTab", 130)
local UniversalTab = createTab("UniversalTab", 165)
local SettingsTab = createTab("SettingsTab", 200)
...
local function createToggle(parent, textKey, default, callback)
    ...
    frame.Size = UDim2.new(0.95, 0, 0, 38); frame.BackgroundColor3 = Color3.fromRGB(20, 20, 24); Instance.new("UICorner", frame).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", frame).Color = Color3.fromRGB(28, 28, 32)
...
local WidgetTg = Instance.new("TextLabel", DragWidget); WidgetTg.Size = UDim2.new(1, 0, 0, 20); WidgetTg.Position = UDim2.new(0, 10, 0, 6); WidgetTg.BackgroundTransparency = 1; WidgetTg.Text = "t.me/LITEVISUALS_OFFICIAL"; WidgetTg.Font = Enum.Font.GothamBold; WidgetTg.TextSize = 10; WidgetTg.TextColor3 = Color3.fromRGB(90, 160, 255); WidgetTg.TextXAlignment = Enum.TextXAlignment.Left
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if settingsData.rgb then local rainbowColor = Color3.fromHSV((tick() % 4) / 4, 0.6, 1); MainStroke.Color = rainbowColor; WidgetStroke.Color = rainbowColor; ALStroke.Color = rainbowColor; StatusIndicator.BackgroundColor3 = rainbowColor
...
-- СКИН ХАК
local function applySkinHack(state)
    settingsData.skinHack = state; saveConfig()
    local char = LocalPlayer.Character; local hum = char and char:FindFirstChildOfClass("Humanoid")
    if not hum then return end
    if state then
        local desc = Instance.new("HumanoidDescription")
        desc.Shirt = 13589886754; desc.Pants = 13589895180; desc.Face = 15334468641
        desc.HairAccessory = ""; desc.HatAccessory = ""
        pcall(function() hum:ApplyDescription(desc) end)
        for _, item in pairs(char:GetChildren()) do if item:IsA("Accessory") then item:Destroy() end end
    else hum.Health = 0 end
end

-- СИСТЕМА УНИВЕРСАЛЬНЫХ МОДИФИКАЦИЙ И ФИЗИКИ
local origAmbient, origColorShift = Lighting.Ambient, Lighting.ColorShift_Top

UserInputService.JumpRequest:Connect(function()
    if settingsData.infJump then
...
RunService.Stepped:Connect(function()
    ...
    if hrp and hum then
        if settingsData.speedHackActive and hum.WalkSpeed ~= settingsData.walkSpeed then hum.WalkSpeed = settingsData.walkSpeed end
        
        -- УЛЬТРА ИСПРАВЛЕННЫЙ ANTI-FLING (ПОЛНАЯ ИЗОЛЯЦИЯ ОТ ЧУЖОЙ ФИЗИКИ)
        if settingsData.antiFling then 
            pcall(function()
        ...
        if settingsData.bHop and UserInputService:IsKeyDown(Enum.KeyCode.Space) and hum.FloorMaterial ~= Enum.Material.Air then
            hrp.Velocity = Vector3.new(hrp.Velocity.X, hum.JumpPower, hrp.Velocity.Z)
        end
        if settingsData.magnetEnabled then
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    local tHrp = p.Character.HumanoidRootPart
                    tHrp.Size = Vector3.new(settingsData.magnetSize, settingsData.magnetSize, settingsData.magnetSize)
                    tHrp.Transparency = 0.7; tHrp.Material = Enum.Material.ForceField; tHrp.CanCollide = false
                end
            end
        end
        if settingsData.spinActive then hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(settingsData.spinSpeed), 0) end
        if settingsData.noclip then
...
RunService.RenderStepped:Connect(function()
    if settingsData.fullbright then
    ...
    while task.wait(5) do
        if settingsData.spammer then
            ...
            if chatService and chatService.ChatVersion == Enum.ChatVersion.TextChatService then
                ...
                if channel then channel:SendAsync("LITE VISUALS on top!") end
            else
                local re = game:GetService("ReplicatedStorage"):FindFirstChild("DefaultChatSystemChatEvents") or game:GetService("ReplicatedStorage"):FindFirstChild("SayMessageRequest")
                if re and re:FindFirstChild("SayMessageRequest") then
                    re.SayMessageRequest:FireServer("LITE VISUALS on top!", "All")
                end
            end
        end
    end
...
-- КОНФИГУРАЦИЯ ВКЛАДКИ MAIN
local walkSlider; walkSlider, _ = createAttachedSlider(MainTab, "WalkSpeed", 16, 150, settingsData.walkSpeed, function(val) settingsData.walkSpeed = val; saveConfig(); if settingsData.speedHackActive and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = val end end)
walkSlider.Visible = false; createToggle(MainTab, "Speedhack", settingsData.speedHackActive, function(state) settingsData.speedHackActive = state; saveConfig(); walkSlider.Visible = state; if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = state and settingsData.walkSpeed or 16 end end)

local flySlider; flySlider, _ = createAttachedSlider(MainTab, "FlySpeed", 20, 150, settingsData.flySpeed, function(val) settingsData.flySpeed = val; saveConfig() end)
flySlider.Visible = false; local flyLoop, keysPressed = nil, {}

local function startFlyLogic(char)
    if flyLoop then flyLoop:Disconnect() end
    if not char or not char:WaitForChild("HumanoidRootPart", 5) or not settingsData.flyActive then return end
    ...
    flyLoop = RunService.RenderStepped:Connect(function()
        if not char:FindFirstChild("HumanoidRootPart") or not bv.Parent then return end; bg.CFrame = Camera.CFrame; local direction = Vector3.new(0, 0, 0)
        ...
        bv.Velocity = direction.Magnitude > 0 and direction.Unit * settingsData.flySpeed or Vector3.new(0,0,0)
    end)
end
...
createToggle(MainTab, "Fly", settingsData.flyActive, function(state)
    settingsData.flyActive = state; saveConfig(); flySlider.Visible = state
    local char = LocalPlayer.Character
    if state then startFlyLogic(char)
    else if flyLoop then flyLoop:Disconnect() end if char and char:FindFirstChild("HumanoidRootPart") then if char.HumanoidRootPart:FindFirstChild("LVFlyBV") then char.HumanoidRootPart.LVFlyBV:Destroy() end if char.HumanoidRootPart:FindFirstChild("LVFlyBG") then char.HumanoidRootPart.LVFlyBG:Destroy() end end end
end)

local spinSlider; spinSlider, _ = createAttachedSlider(MainTab, "SpinSpeed", 10, 150, settingsData.spinSpeed, function(val) settingsData.spinSpeed = val; saveConfig() end)
spinSlider.Visible = false; createToggle(MainTab, "SpinBot", settingsData.spinActive, function(state) settingsData.spinActive = state; saveConfig(); spinSlider.Visible = state end)
createToggle(MainTab, "Noclip", settingsData.noclip, function(state) settingsData.noclip = state; saveConfig() end)
createToggle(MainTab, "AntiFling", settingsData.antiFling, function(state) settingsData.antiFling = state; saveConfig() end)

local magnetSlider; magnetSlider, _ = createAttachedSlider(MainTab, "Magnet", 2, 50, settingsData.magnetSize, function(val) settingsData.magnetSize = val; saveConfig() end)
magnetSlider.Visible = false; createToggle(MainTab, "Magnet", settingsData.magnetEnabled, function(state)
    settingsData.magnetEnabled = state; saveConfig(); magnetSlider.Visible = state
    if not state then for _, p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then local hrp = p.Character.HumanoidRootPart; hrp.Size = Vector3.new(2, 2, 1); hrp.Transparency = 0; hrp.Material = Enum.Material.Plastic end end end
end)

-- НАПОЛНЕНИЕ ВКЛАДКИ UNIVERSAL
local LabelTP = Instance.new("TextLabel", UniversalTab); LabelTP.Size = UDim2.new(0.95, 0, 0, 30); LabelTP.BackgroundTransparency = 1; LabelTP.Font = Enum.Font.GothamBold; LabelTP.TextSize = 12; LabelTP.TextColor3 = Color3.fromRGB(90, 160, 255); LabelTP.TextXAlignment = Enum.TextXAlignment.Center; registerText(LabelTP, "ClickTP")
createToggle(UniversalTab, "InfJump", settingsData.infJump, function(state) settingsData.infJump = state; saveConfig() end)
createToggle(UniversalTab, "BHop", settingsData.bHop, function(state) settingsData.bHop = state; saveConfig() end)
createToggle(UniversalTab, "Fullbright", settingsData.fullbright, function(state) settingsData.fullbright = state; saveConfig(); if not state then Lighting.Ambient = origAmbient; Lighting.ColorShift_Top = origColorShift end end)
createToggle(UniversalTab, "Spammer", settingsData.spammer, function(state) settingsData.spammer = state; saveConfig() end)

-- ========================================================
-- УЛЬТРА СКОРОСТНАЯ СИСТЕМА ОБНАРУЖЕНИЯ РОЛЕЙ MM2 ДЛЯ ESP
-- ========================================================
...
local function checkMM2Role(player)
    ...
    if knife then return Color3.fromRGB(255, 30, 30), "MURDER" end 
    ...
    if gun then return Color3.fromRGB(30, 144, 255), "SHERIFF" end 
...
local function applyESP(player)
    ...
    local function init()
        ...
        if espFolder:FindFirstChild(player.Name.."_TXT") then espFolder[player.Name.."_TXT"]:Destroy() end
        
        local high = Instance.new("Highlight", espFolder); high.Name = player.Name; high.Adornee = char; high.FillTransparency = 0.55
        local bill = Instance.new("BillboardGui", espFolder); bill.Name = player.Name.."_TXT"; bill.Adornee = head; bill.Size = UDim2.new(0, 130, 0, 40); bill.StudsOffset = Vector3.new(0, 3, 0); bill.AlwaysOnTop = true
        local txt = Instance.new("TextLabel", bill); txt.Size = UDim2.new(1,0,1,0); txt.BackgroundTransparency = 1; txt.Font = Enum.Font.GothamBold; txt.TextSize = 11; txt.TextColor3 = Color3.new(1,1,1); txt.TextStrokeTransparency = 0.2
        
        local connection
        connection = RunService.RenderStepped:Connect(function()
            if not settingsData.espEnabled or not char or not char.Parent or not char:FindFirstChild("Humanoid") or char.Humanoid.Health <= 0 then
                connection:Disconnect(); high:Destroy(); bill:Destroy(); return
            end
            
            local color
            local mm2Color, role = checkMM2Role(player)
            
            if settingsData.espMM2 and role ~= "INNOCENT" then
                color = mm2Color
            else
                local wave = (math.sin(tick() * 2.5) + 1) / 2
                color = Color3.fromRGB(138, 43, 226):Lerp(Color3.fromRGB(0, 191, 255), wave)
            end
            
            high.FillColor = color; high.OutlineColor = color
            
            local labelStr = ""
            if settingsData.espNames then labelStr = player.DisplayName end
            if settingsData.espMM2 and role ~= "INNOCENT" then 
                labelStr = (labelStr ~= "" and labelStr .. "\n" or "") .. "[" .. role .. "]"
            end
            
            txt.Text = labelStr; txt.TextColor3 = color
            bill.Enabled = (settingsData.espNames or (settingsData.espMM2 and role ~= "INNOCENT"))
        end)
    end
    
    player.CharacterAdded:Connect(init)
    if player.Character then task.spawn(init) end
end

createToggle(VisualsTab, "Esp", settingsData.espEnabled, function(state)
    settingsData.espEnabled = state; saveConfig()
    if state then 
...
createToggle(VisualsTab, "EspNames", settingsData.espNames, function(state) settingsData.espNames = state; saveConfig() end)
createToggle(VisualsTab, "EspMM2", settingsData.espMM2, function(state) settingsData.espMM2 = state; saveConfig() end)
createToggle(VisualsTab, "SkinHack", settingsData.skinHack, function(state) applySkinHack(state) end)
createAttachedSlider(VisualsTab, "Fov", 70, 120, settingsData.fov, function(val) settingsData.fov = val; saveConfig(); Camera.FieldOfView = val end)

local defaultLighting = {Ambient = Lighting.Ambient, Brightness = Lighting.Brightness, GlobalShadows = Lighting.GlobalShadows}
local CC = Instance.new("ColorCorrectionEffect", Lighting); CC.Enabled = false; CC.Saturation = 0.3; CC.Contrast = 0.1
local Bloom = Instance.new("BloomEffect", Lighting); Bloom.Enabled = false; Bloom.Intensity = 0.3
createToggle(VisualsTab, "Graphics", settingsData.graphics, function(state)
    settingsData.graphics = state; saveConfig(); CC.Enabled = state; Bloom.Enabled = state
    if state then Lighting.Ambient = Color3.fromRGB(150, 150, 150); Lighting.Brightness = 2; Lighting.GlobalShadows = true else Lighting.Ambient = defaultLighting.Ambient; Lighting.Brightness = defaultLighting.Brightness; Lighting.GlobalShadows = defaultLighting.GlobalShadows end
end)

-- КОНФИГУРАЦИЯ ВКЛАДКИ SETTINGS
...
LangBtn.MouseButton1Click:Connect(function() if currentLang == "RU" then updateLanguage("EN") else updateLanguage("RU") end; TopTitle.Text = Localization[currentLang][TabButtons[1].Parent and "MainTab" or "SettingsTab"]; updateActiveMods() end)

createToggle(SettingsTab, "Widget", false, function(state) DragWidget.Visible = state end)
createToggle(SettingsTab, "ActiveList", false, function(state) ActiveListWidget.Visible = state end)
createToggle(SettingsTab, "Rainbow", settingsData.rgb, function(state) settingsData.rgb = state; saveConfig() end)
createToggle(SettingsTab, "AutoSave", settingsData.autoSave, function(state) settingsData.autoSave = state; saveConfig() end)

-- ХЕРАТБИТ ДЛЯ СКОРОСТИ, ФЛАЯ И СКИНОВ
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local char = LocalPlayer.Character
            if char and char:FindFirstChildOfClass("Humanoid") and char.Humanoid.Health > 0 then
                if settingsData.speedHackActive and char.Humanoid.WalkSpeed ~= settingsData.walkSpeed then char.Humanoid.WalkSpeed = settingsData.walkSpeed end
                if settingsData.flyActive and char:FindFirstChild("HumanoidRootPart") and (not char.HumanoidRootPart:FindFirstChild("LVFlyBV") or not char.HumanoidRootPart:FindFirstChild("LVFlyBG")) then startFlyLogic(char) end
                if settingsData.skinHack and not char:FindFirstChild("LV_SkinApplied") then Instance.new("BoolValue", char).Name = "LV_SkinApplied"; applySkinHack(true) end
            end
            if settingsData.fullbright and Lighting.Ambient ~= Color3.fromRGB(255, 255, 255) then Lighting.Ambient = Color3.fromRGB(255, 255, 255); Lighting.ColorShift_Top = Color3.fromRGB(255, 255, 255) end
        end)
    end
end)

task.spawn(function()
    if settingsData.espEnabled then
        for _, p in pairs(Players:GetPlayers()) do applyESP(p) end
    end
end)

updateLanguage("RU"); TopTitle.Text = Localization[currentLang]["MainTab"]; updateActiveMods()
Camera.FieldOfView = settingsData.fov
...
print("Lite Visuals Premium Universal Suite Loaded Successfully.")
