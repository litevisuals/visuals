...
local Localization = {
    RU = {
        ...
        Fly = "Fly (По камере)", FlySpeed = "Скорость полета", SpinBot = "Spin Bot", SpinSpeed = "Скорость вращения",
    ...
    EN = {
        ...
        Fly = "Fly (Camera)", FlySpeed = "Fly Speed", SpinBot = "Spin Bot", SpinSpeed = "Spin Speed",
...
MobileToggle.Size = UDim2.new(0, 45, 0, 45); MobileToggle.Position = UDim2.new(0.02, 0, 0.2, 0); MobileToggle.BackgroundColor3 = Color3.fromRGB(20, 20, 25); MobileToggle.Text = "LITE"; MobileToggle.TextColor3 = Color3.new(1, 1, 1); MobileToggle.Font = Enum.Font.GothamBold; MobileToggle.TextSize = 10; MobileToggle.ZIndex = 10
Instance.new("UICorner", MobileToggle).CornerRadius = UDim.new(1, 0)
local MobileStroke = Instance.new("UIStroke", MobileToggle); MobileStroke.Color = Color3.fromRGB(50, 50, 60); MobileStroke.Thickness = 1.5
...
-- УЛУЧШЕННЫЙ ПОЛЗУНОК ДЛЯ ОШИБОК СЕНСОРНОГО ТАЧА (MOUSE + TOUCH ВЗАИМОДЕЙСТВИЕ)
local function createAttachedSlider(parent, textKey, min, max, default, callback)
    ...
    slideBg.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then dragging = true; update(i) end end)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if rgbEffectsEnabled then 
        ...
        MainStroke.Color = rainbowColor; WidgetStroke.Color = rainbowColor; ALStroke.Color = rainbowColor; StatusIndicator.BackgroundColor3 = rainbowColor; MobileStroke.Color = rainbowColor
    else 
        MainStroke.Color = Color3.fromRGB(32, 32, 36); WidgetStroke.Color = Color3.fromRGB(30, 30, 35); ALStroke.Color = Color3.fromRGB(50, 50, 55); StatusIndicator.BackgroundColor3 = Color3.fromRGB(75, 255, 100); MobileStroke.Color = Color3.fromRGB(50, 50, 60)
    end
end)

local orbitAngle, flyEnabledState = 0, false
...
-- МОБИЛЬНЫЙ FLY: ЛЕТИТ АВТОМАТИЧЕСКИ ТУДА, КУДА СМОТРИТ КАМЕРА ПРИ ИСПОЛЬЗОВАНИИ СЕНСОРНОГО ДЖОЙСТИКА
...
createToggle(MainTab, "Fly", false, function(state)
    local char = LocalPlayer.Character; flySlider.Visible = state; flyEnabledState = state
    if state then
        ...
        flyLoop = RunService.RenderStepped:Connect(function()
            ...
            -- Если на мобилке задействован виртуальный джойстик направления (MoveDirection > 0)
            if hum and hum.MoveDirection.Magnitude > 0 then
                bv.Velocity = Camera.CFrame.LookVector * flySpeed
            else
                bv.Velocity = Vector3.new(0, 0.1, 0) -- Зависание на месте без просадки высоты
            end
        end)
    else
...
local function toggleMenu()
    if isMenuOpen then 
        MainCanvas:TweenSize(UDim2.new(0, 520, 0, 0), "Out", "Quart", 0.3, true, function() MainCanvas.Visible = false end)
        isMenuOpen = false
    else 
        MainCanvas.Visible = true
        MainCanvas:TweenSize(UDim2.new(0, 520, 0, 330), "Out", "Back", 0.3, true)
        isMenuOpen = true 
    end
end
...
print("Mobile version with directional camera fly loaded successfully.")
